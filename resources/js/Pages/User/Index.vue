...
<template>
    <app-layout>
        <div class="card">
            ...
            <Edit :show="data.editOpen" @close="data.editOpen = false" :roles="roles" :user="data.user"
                :title="props.title" />
            <Button v-show="can(['create user'])" label="Create" @click="data.createOpen = true" icon="pi pi-plus" />
            <DataTable lazy :value="users.data" paginator :rows="users.per_page" :totalRecords="users.total"
                :first="(users.current_page - 1) * users.per_page" @page="onPageChange" tableStyle="min-width: 50rem">
                <template #header>
            ...
            <Dialog v-model:visible="deleteDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                <div class="flex items-center gap-4">
                    ...
                    <span v-if="data.user">Are you sure you want to delete <b>{{ data.user.name }}</b>?</span>
                </div>
                <template #footer>
