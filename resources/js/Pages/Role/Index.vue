...
<template>
    <app-layout>
        <div class="card">
            ...
            <Edit :show="data.editOpen" @close="data.editOpen = false" :role="data.role" :title="props.title"
                :permissions="props.permissions" />
            <Button v-show="can(['create role'])" label="Create" @click="data.createOpen = true" icon="pi pi-plus" />
            <DataTable lazy :value="roles.data" paginator :rows="roles.per_page" :totalRecords="roles.total"
                :first="(roles.current_page - 1) * roles.per_page" @page="onPageChange" tableStyle="min-width: 50rem">
                <template #header>
                ...
                <Column header="Permission">
                    <template #body="slotProps">
                        <div @click="(permissionDialog = true), (data.role = slotProps.data)"
                ...
                <Column :exportable="false" style="min-width: 12rem">
                    <template #body="slotProps">
                        ...
                        <Button v-show="can(['delete role'])" icon="pi pi-trash" outlined rounded severity="danger"
            ...
            <Dialog v-model:visible="deleteDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                <div class="flex items-center gap-4">
                    ...
                    <span v-if="data.role">Are you sure you want to delete <b>{{ data.role.name }}</b>?</span>
                </div>
                <template #footer>
            ...
            <Dialog v-model:visible="permissionDialog" modal :header="'Permission ' + data.role?.name"
                :style="{ width: '50rem' }">
                <div class="grid grid-cols-2 sm:grid-cols-3">
                    <div v-for="(permission, index) in data.role?.permissions" :key="index"
